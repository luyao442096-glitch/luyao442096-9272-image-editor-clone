// 最终版测试积分充值脚本 - 支持多个auth token cookie
// 使用说明：
...
async function testCreditsAddition(plan) {
    ...
    // 从cookie中获取访问令牌 - 支持多个auth token cookie
    function getAuthToken() {
        // 查找所有以sb-开头的auth token cookie
        const cookies = document.cookie.split('; ');
        const authCookies = cookies.filter(cookie => 
            cookie.startsWith('sb-mqhqofqmvfgfslclnini-auth-token')
        );
        ...
        for (const authCookie of authCookies) {
            try {
                ...
                if (tokenData && tokenData.access_token) {
                    console.log(`✅ 成功解析auth token cookie`);
